    ...
    installInto: function (expect) {
        expect
            ...
            if (requestProperties.formData) {
                if (httpRequest.body) {
                    return done(new Error('unexpected-express: The "body" and "formData" options are not supported together'));
                }
                httpRequest.body = new FormData();
                Object.keys(requestProperties.formData).forEach(function (name) {
                    ...
                    httpRequest.body.append(name, value, options);
                });
                ...
                if (httpRequest.body.pipe) {
                    ...
                    setImmediate(function emitRequestBodyOnceThereIsAListener() {
                        if (req.listeners('data').length > 0 || req.listeners('end').length > 0) {
                            ...
                            setTimeout(emitRequestBodyOnceThereIsAListener, 1);
                        }
                    });
                }
            } else {
            ...
            res.connection.cork = res.connection.uncork = function () {};
...
//            res.connection.resume();

            var originalEnd = res.connection.end;
            res.connection.end = function () {
                        ...
                        if (typeof expectedMetadata.errorPassedToNext !== 'undefined') {
                            ...
                            if (!res.getHeader('Content-Type')) {
                                ...
                                if (rawResponseChunks.length === 0) {
                                    rawResponseChunks.push(
                                        new Buffer(
                                            'HTTP/1.1 ' + (errorPassedToNext.statusCode || 500) + '\r\n' +
                                            'Content-Type: text/plain; charset=UTF-8\r\n' +
                                            '\r\n' +
                                            errorPassedToNext.stack,
                                            'utf-8'
                                        )
                                    );
                                }
            ...
            var hasEnded = false; // Keep track of whether an additional res.write() calls is the result of res.end(chunk)
